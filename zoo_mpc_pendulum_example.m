...
Kmpc = Kmpc( Ksysid ,...
        'horizon' , 2 ,...
        'input_bounds' , [-pi,pi]/4,... %[ -7*pi/8 , 7*pi/8 ] ,... % DO NOT USE INF
        'input_slopeConst' , [1e-5],... %1e-1 ,...
...
%           linspace( 0 , -7*pi/8 , num_steps )' ];
      
% ramp and hold
u1 = 7*pi/8;
u2 = -7*pi/8;
u_temp = [linspace( 0 , u1 , ceil(num_steps/2) )' ,...
          linspace( 0 , u2 , ceil(num_steps/2) )' ;
          ones( floor(num_steps/2) , 1 ) * u1 ,...
          ones( floor(num_steps/2) , 1 ) * u2];
